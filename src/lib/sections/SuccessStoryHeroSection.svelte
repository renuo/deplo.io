...
>
  <section class="container relative flex h-full flex-col justify-end pb-16 text-deploio">
    <div class="relative flex h-full w-full flex-col justify-end text-white">
      <div class="mb-8">
        <h1 class="text-h1 mb-4 max-w-[450px] md:max-w-[800px]" use:appear={{ delay: 0 }}>
          {m.success_story_title()}
        </h1>
        <p class="mb-10 max-w-[800px] text-[20px]" use:appear={{ delay: 100 }}>{m.success_story_long_description()}</p>

        <div class="mb-3 flex flex-wrap gap-2 md:gap-6" use:appear={{ delay: 200 }}>
          <div>
            <span>Client</span>
            <p class="opacity-80">RenuoAG</p>
          </div>
