<script lang="ts">
  ...
  import successStoryBackground from '$lib/assets/backgrounds/swissict_success_story_background.webp';
  import corneliaAvatar from '$lib/assets/avatars/swissict_cornelia.webp';

  const clientName = 'swissICT';
</script>

<svelte:head>
  <title>{m.meta_title_success_story({ clientName })}</title>
  <meta name="description" content={m.success_story_swissict_description()} />
  <GoogleTag />
</svelte:head>
...
<main class="relative z-0">
  <SuccessStoryHeroSection
    ...
    description={m.success_story_swissict_long_description()}
    {clientName}
    clientWebsite="swissict.ch"
    backgroundImage={successStoryBackground}
  />
  <SuccessStoryMainSection
    title1={m.success_story_swissict_challenge_title()}
    text1={m.success_story_swissict_challenge_text()}
    title2={m.success_story_swissict_solution_title()}
    text2={m.success_story_swissict_solution_text()}
  >
    <div
    ...
    >
      <span class="text-2xl">
        {m.success_story_swissict_quote_text()}
      </span>
      <div class="mt-6 flex items-center font-normal">
        <div class="border-t-1 mr-4 flex-shrink-0 border-deploio">
          <img
            src={corneliaAvatar}
            alt={m.success_story_swissict_quote_name()}
            class="m-2 h-full w-11 rounded-full bg-gray-200 object-cover"
          />
        </div>
        <div class="text-base">
          <p use:appear={{ delay: 350 }}>{m.success_story_swissict_quote_name()}</p>
          <p use:appear={{ delay: 400 }} class="opacity-70">{m.success_story_swissict_quote_role()}</p>
        </div>
      </div>
    </div>
  </SuccessStoryMainSection>
  <SuccessStoryNotesSection title={m.success_story_swissict_note_title()} text={m.success_story_swissict_note_text()} />
  <RegisterSection />
</main>
